...
const DashboardPreview: React.FC = () => {
  ...
  return (
    <div className="relative rounded-t-3xl border border-gray-200 dark:border-white/10 bg-[#0A0A0A] dark:bg-[#0A0A0A]/80 backdrop-blur-xl shadow-2xl overflow-hidden animate-float" style={{ animationDuration: '8s' }}>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
        <div className="flex items-center gap-8">
            <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-gradient-to-br from-brand-orange to-orange-700 flex items-center justify-center">
                    <span className="font-bold text-white text-[10px]">KN</span>
                </div>
                <span className="font-semibold text-white">KN Analytics</span>
            </div>
        ...
        <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/5 text-gray-400 text-sm w-64">
                <Search size={14} />
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-gray-700 to-gray-600 flex items-center justify-center text-xs font-medium text-white ring-2 ring-black">
                JD
            </div>
        </div>
      </div>
      ...
      {/* Dashboard Body */}
      <div className="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto min-h-[300px]">
        
        {/* Left Col - Top Products Table */}
        <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-white">{t('dashboard.top_products')}</h3>
                <button className="text-xs text-brand-orange hover:text-orange-400">{t('dashboard.view_all')}</button>
            </div>
            
            <div className="bg-white/5 rounded-xl border border-white/5 overflow-hidden">
                <div className="grid grid-cols-12 gap-4 p-4 text-xs font-medium text-gray-500 border-b border-white/5 uppercase tracking-wider">
                ...
                {[1, 2, 3, 4].map((item) => (
                    <div key={item} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-white/5 transition-colors border-b border-white/5 last:border-0 group">
        ...
        <div className="space-y-6">
             <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-white">{t('dashboard.visitors')}</h3>
            </div>
            
            <div className="bg-white/5 rounded-xl border border-white/5 p-6 h-full min-h-[250px] relative overflow-hidden group flex flex-col">
                 <div className="flex flex-col gap-1 mb-8">
                    <span className="text-3xl font-bold text-white">24.5k</span>
