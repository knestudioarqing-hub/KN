...
const Testimonials: React.FC = () => {
  ...
  return (
    <section id="depoimentos" className="relative py-16 sm:py-32 bg-brand-secondary dark:bg-black overflow-hidden border-t border-gray-200 dark:border-white/5 transition-colors duration-500">
      ...
      <div className="absolute top-20 left-10 w-[300px] h-[300px] bg-brand-accent1/10 blur-[80px] rounded-full pointer-events-none"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 sm:mb-20 max-w-4xl mx-auto">
          <h2 className="text-2xl sm:text-5xl font-bold font-sans tracking-tight antialiased text-gray-900 dark:text-white mb-4 sm:mb-6 transition-colors">
            {t('testimonials.title')} <br />
            <span style={{ color: '#22c55e' }}>{t('testimonials.titleHighlight')}</span>
          </h2>
          <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-lg transition-colors">{t('testimonials.subtitle')}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8">
          {testimonials.map((item: any, i: number) => (
            <div key={i} className="group relative p-5 sm:p-8 rounded-2xl bg-white dark:bg-[#0A0A0A] border border-gray-200 dark:border-white/5 hover:border-brand-accent1/20 transition-all duration-300 hover:-translate-y-1 overflow-hidden shadow-lg dark:shadow-none">
